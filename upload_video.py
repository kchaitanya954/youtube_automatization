...
from generate_video import GenrateVideo
...
def upload_video(video_path, title, description, category, keywords, privacy_status='public'):
    try:
...
if __name__ == '__main__':
    video = GenrateVideo()
    video_info = video.create_video()
    try:
        video_path=video_info['output_path']
        title = video_info['Title']
        description=video_info['script']
        keywords = video_info['Keywords']
        category = video_info['Category Number']
        try:
            upload_video(video_path, title, description, category, keywords)
        except Exception:
            print(Exception)
        
        # Remove the video file after upload
        if os.path.exists(video_path):
            os.remove(video_path)
            print(f"Video file {video_path} has been successfully removed.")
        else:
